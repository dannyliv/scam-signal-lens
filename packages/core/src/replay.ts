...
import { buildPassARequest, buildPassBRequest, type CorpusQuestionScope, type SystemOneRequest } from "./questions.js";
import { derivePolicy, NO, policyFingerprint, signalStatus } from "./policy.js";
...
export function deriveReplay(input: ModelInput, language: "en" | "es", segments: Segment[], passA: PublicProviderResponse, evidence: ReplayRecord["derived"]["evidence"]): ReplayRecord["derived"] {
  validateSegments(input, segments);
  const request = buildPassARequest(input, language);
  ...
  const policy = derivePolicy({ signals, messageRole: contextual("message_role"), requestRoute: contextual("request_route") });
...
export async function verifyReplayRecord(record: ReplayRecord, input: ModelInput, language: "en" | "es"): Promise<ReplayRecord["derived"]> {
  ...
  if (record.policySha256 !== await sha256Web(policyFingerprint())) throw new Error("recording policy hash mismatch");
  ...
  const passARequest = buildPassARequest(input, language, scope);
  if (record.requestedModel !== passARequest.model) throw new Error("recording requested model mismatch");
  if (record.questionBundleSha256 !== await sha256Web(passARequest.questions)) throw new Error("recording question hash mismatch");
  if (record.passA.requestSha256 !== await sha256Web(passARequest)) throw new Error("Pass A request hash mismatch");
  ...
  const plan = buildPassBRequest(input, record.segments, eligible, language, scope);
  ...
  if (plan.request === null) {
    ...
    if (record.passB.requestSha256 !== await sha256Web(plan.request)) throw new Error("Pass B request hash mismatch");
  ...
  const derived = deriveReplay(input, language, record.segments, allowlistedProviderResponse(record.passA.response, expectedQuestions(passARequest)), evidence);
