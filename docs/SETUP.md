...
Thresholds, signal questions, input limits, the requested model, the TypeSafe endpoint, the attempt cap, and segmentation are source constants. [Editing policy, thresholds, and inputs](../README.md#editing-policy-thresholds-and-inputs) names the files. Changing one of them means `pnpm build:recorder` and a new private `--out` directory and `--run-id`.
