...
The evaluator makes no provider request. It verifies manifest and configuration bindings, checks every available replay record with the core verifier, and writes JSON, CSV, and readable reports. A verifier-valid Pass A record remains in a private report when Pass B evidence failed, and its CSV row has `capture_complete=false`. Missing or invalid Pass A records are unavailable.
