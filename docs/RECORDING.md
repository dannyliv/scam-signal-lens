...
Thresholds, signal questions, input limits, `REQUESTED_MODEL`, the TypeSafe endpoint, the attempt cap, and segmentation are source constants. [Editing policy, thresholds, and inputs](../README.md#editing-policy-thresholds-and-inputs) names the files. None of them is a recorder flag. An edit needs `pnpm build:recorder` and a new `--out` directory and `--run-id`, because an existing run directory keeps its configuration.
